...
/**
 * The XMPPFileManager is responsible for receiving files from
 * Smack users.
 * 
 */
...
public class XMPPFileManager {
	...
	*/
	...
	public XMPPFileManager(XMPPConnection con, FileTransferManager ftm, DecimalFormat progress) {
	    this.connection = con;
	...
	public void fileReceiver(final String contact) {
		...
		fileTransferManager.addFileTransferListener(new FileTransferRequestListener());
	}
	
	/**
     * This is an File Transfer Listener class that is responsible for handling user's
     * file receiving.
     */
	...
	private class FileTransferRequestListener implements FileTransferListener {
		...
		public void fileTransferRequest(FileTransferRequest request) {
	    	...
	    	if(request.getRequestor().contains(requester)) {
			    ...
			    int confirmation = JOptionPane.showConfirmDialog(null, "From: " + requester + "\nDescription: Sending a file" + "\nFile Name: " + request.getFileName() + " \nFile Size: " + progressBar.format(request.getFileSize()/1024) + " KB");
				...
				if (confirmation == JOptionPane.OK_OPTION) {
			        ...
			        if(confirmedLocation == JFileChooser.APPROVE_OPTION) {
			        	...
			        	try {
                			...
                			while(!transfer.isDone() && progress.counter != 100) {
                				
                				if(transfer.getStatus().equals(Status.error)) {
                					JOptionPane.showMessageDialog(null, "Cannot receive the file because an error occured during the process.",
                	                        "Failed", JOptionPane.ERROR_MESSAGE);
                			    	return;
                				}
                				else
                					System.out.println("Transferring Progress: " + progressBar.format(transfer.getProgress() * 100)  + "% transferred.");
                						    
                				try {
                					Thread.sleep(1500);
                					progress.counter = (int)(transfer.getProgress() * 100);
                				} 
                					...
                					e.printStackTrace();
                						...
                						System.out.println(request.getFileName() + "has been successfully received.");
