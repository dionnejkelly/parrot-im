...
public class CustomizedChatbotModel {
    private Vector<ChatbotQADataType> QAs = new Vector<ChatbotQADataType>();

    public CustomizedChatbotModel(Model model) throws ClassNotFoundException,
            SQLException {
        QAs.clear();
        ...
        if (questionList != null) {
            for (int i = 0; i < questionList.size(); i++) {
                ...
                tempQuestionsList =
                        model.getAllAfterQuestions(questionList.get(i));
                if (!doneQuestionsList.containsAll(tempQuestionsList)) {
                    QAs.add(new ChatbotQADataType(model, tempQuestionsList,
                            tempAnswersList));
    ...
    public void addQA(ChatbotQADataType newQA) throws ClassNotFoundException,
            SQLException {
        QAs.add(newQA);
    }
