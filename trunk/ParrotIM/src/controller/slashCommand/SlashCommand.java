...
public class SlashCommand {
		...
		while (tokenizer.hasMoreTokens() && numberOfTokens<=4){
			...
			if (numberOfTokens==1){
				...
				if (token.compareToIgnoreCase(UserStateType.ONLINE.toString())==0 || //online
						...
						token.compareToIgnoreCase("brb")==0 ) { //brb (be right back)
					hasSetPresence = true;
					break;
				} else {
					token += " ";
				...
				//four tokens
				if (token.compareToIgnoreCase(UserStateType.NOT_BE_DISTURBED.toString())==0){ 
																		//not to be disturbed
					hasSetPresence = true;
					break;
				} else {
	...
	public static void main (String [] args) {
		...
		try {
			...
			//status message only
			System.out.println(cmd.isSlashCommand("/I am changing my status message"));
			System.out.println();
			...
			//status + presence
			System.out.println(cmd.isSlashCommand("/not available I am so awesome"));
			System.out.println();
			
			System.out.println(cmd.isSlashCommand("/brb another brb"));
			System.out.println();
			
			System.out.println(cmd.isSlashCommand("/be right back this is supposed to work"));
			System.out.println();
			
			System.out.println(cmd.isSlashCommand("/not to be disturbed hahaha this is my message!"));
			System.out.println();
